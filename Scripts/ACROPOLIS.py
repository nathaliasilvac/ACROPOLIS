...
def combine_split_zip_files(zip_prefix, target_zip_file):
    ...
    split_zip_files = [file for file in os.listdir(
        model_folder_path) if file.startswith(zip_prefix)]

    # Sort the split zip files based on the suffix number
    split_zip_files.sort(key=lambda x: int(x.split(".")[-1]))

    # Combine the split zip files into a single zip file
    with open(target_zip_file, 'wb') as target_zip:
        for split_zip_file in split_zip_files:
            with open(os.path.join(model_folder_path, split_zip_file), 'rb') as f:
                shutil.copyfileobj(f, target_zip)
...
if not os.path.exists(file_to_check):
    ...
    # Combine the split zip files in the 'ML Model' folder
    combine_split_zip_files(zip_file_prefix, os.path.join(model_folder_path, merged_zip_file))

    # Extract the combined zip file into the current directory
    try:
        patoolib.extract_archive(os.path.join(model_folder_path, merged_zip_file))
        print("File 'ACROPOLIS_ML.sav' has been extracted successfully.")
    except patoolib.util.PatoolError as e:
        print("Error occurred while extracting.")
        print(str(e))
    else:
        # Remove the merged zip file after extraction
        os.remove(os.path.join(model_folder_path, merged_zip_file))
        print("Merged zip file 'ACROPOLIS_ML_merged.zip' has been removed.")
else:
    print("File 'ACROPOLIS_ML.sav' already exists.")
...
def ascii_to_tiff(ascii_file_path, tiff_file_path):
    ...
    transform = rasterio.transform.from_origin(
        xllcenter, yllcenter + (cellsize * height), cellsize, cellsize)
...
def dem():
    ...
    ax1.scatter(coorbalsa.balsa[0], coorbalsa.balsa[1],
                color='red', marker='v', s=150, label='Balsa')
    ax1.legend()
        ...
        def locaff():
            ...
            class loccoor():
                ...
                filepath = filedialog.askopenfilename(
                    filetypes=[("CSV files", ".csv")], title="Open file")
            ...
            class plot():
                ...
                ax1.scatter(coorbalsa.balsa[0], coorbalsa.balsa[1],
                            color='red', marker='v', label='Reservoir', s=150)
                ax1.legend()
                ax1.grid()
            ...
            def ejes():
                ...
                if not rotacion:
                    ...
                    ax1.scatter(mainc.mainchannel['x'], mainc.mainchannel['y'],
                                s=15, label='Main channel', color='blue')
                    ax1.scatter(xbalsa, ybalsa, s=150, marker='v', label='Reservoir', color='red')
                    ax1.plot(ejebalsa['x'], ejebalsa['y'],
                             label='Reservoir axis', linestyle='--', color='black')
                    ax1.plot(ejebrecha['x'], ejebrecha['y'],
                             label='Breach axis', linestyle='--', color='green')
                    ax1.grid()
                    ax1.legend(fontsize=14)
                    ax1.set_title('Localization AoI, axes and channel',
                ...
                def distancias():
                    ...
                    ax1.scatter(nograve['x'], nograve['y'], marker='^',
                                color='blue', s=150, label='No Risk')
                    ax1.scatter(grave['x'], grave['y'], marker='^',
                                color='red', s=120, label='Risk')

                    ax1.scatter(xbalsa, ybalsa, s=150, marker='v',
                                label='Reservoir', color='black')
                    ax1.plot(ejebalsa['x'], ejebalsa['y'],
                             label='Reservoir axis', linestyle='--', color='black')
                    ax1.plot(ejebrecha['x'], ejebrecha['y'],
                             label='Breach axis', linestyle='--', color='green')
                    ...
                    cat_label = tk.Label(
                        frame, text='Off-stream reservoir Category', font=('calibre', 10, 'bold'))
                    ...
                    tk.Label(frame, text='Risk classification', font=(
                        'calibre', 12, 'bold')).grid(row=12, column=2)
                    tk.Label(frame, text='Type AoI', font=(
                        'calibre', 12, 'bold')).grid(row=12, column=3)
                    ...
                    def esto():
                        ...
                        def nextt():
                            ...
                            tk.Label(frame, text='Iterations (%)', font=(
                                'calibre', 12, 'bold')).grid(row=12, column=4)
                            p = 0.05
                            ...
                            for ll in range(0, loop):
                                Bln = random.uniform((Bl.get()-(Bl.get()*p)),
                                                     (Bl.get()+(Bl.get()*p)))
                                Slbn = random.uniform((Slb.get()-(Slb.get()*p)),
                                                      (Slb.get()+(Slb.get()*p)))
                                Stn = random.uniform((St.get()-(St.get()*p)),
                                                     (St.get()+(St.get()*p)))
                                Slcn = random.uniform((Slc.get()-(Slc.get()*p)),
                                                      (Slc.get()+(Slc.get()*p)))
                                Lbn = random.uniform((Lb.get()-(Lb.get()*p)),
                                                     (Lb.get()+(Lb.get()*p)))
                                Wcn = random.uniform((Wc.get()-(Wc.get()*p)),
                                                     (Wc.get()+(Wc.get()*p)))
                                Wrn = random.uniform((Wr.get()-(Wr.get()*p)),
                                                     (Wr.get()+(Wr.get()*p)))
                                Lrn = random.uniform((Lr.get()-(Lr.get()*p)),
                                                     (Lr.get()+(Lr.get()*p)))
                                Hrn = random.uniform((Hr.get()-(Hr.get()*p)),
                                                     (Hr.get()+(Hr.get()*p)))
                                voln = random.uniform(
                                    (voll.get()-(voll.get()*p)), (voll.get()+(voll.get()*p)))
                                Hcn = random.uniform((Hc.get()-(Hc.get()*p)),
                                                     (Hc.get()+(Hc.get()*p)))
                                mann = random.uniform(
                                    (Manning.get()-(Manning.get()*p)), (Manning.get()+(Manning.get()*p)))
                                for kk in range(0, n):
                                ...
                                new_malla = pd.concat([pd.DataFrame(np.full(n, Bln), columns=['Damloc']),
                                                       pd.DataFrame(np.full(n, Slbn),
                                                                    columns=['slope_IA']),
                                                       pd.DataFrame(np.full(n, Stn),
                                                                    columns=['trans_slope']),
                                                       pd.DataFrame(np.full(n, Slcn),
                                                                    columns=['long_slope']),
                                                       pd.DataFrame(np.full(n, Lbn),
                                                                    columns=['initial_A']),
                                                       pd.DataFrame(np.full(n, Wcn), columns=[
                                                                    'Wchannel_bottom']),
                                                       pd.DataFrame(np.full(n, Wrn),
                                                                    columns=['w_dam']),
                                                       pd.DataFrame(np.full(n, Lrn),
                                                                    columns=['l_dam']),
                                                       pd.DataFrame(np.full(n, Hrn),
                                                                    columns=['h_dam']),
                                                       pd.DataFrame(np.full(n, voln),
                                                                    columns=['vol']),
                                                       pd.DataFrame(np.full(n, Hcn),
                                                                    columns=['hchannel']),
                                                       pd.DataFrame(np.full(n, mann),
                                                                    columns=['manning']),
                                                       dis
                                                       ], axis=1)
                            ...
                            for k in range(0, len(classprediction)):
                                ...
                                perc = per[k]
                                if r == 'Risk':
                                    tk.Label(frame, text=round(perc, 2), font=(
                                        'calibre', 12), fg='black').grid(row=k+13, column=4)
                                if r == 'No Risk':
                                    tk.Label(frame, text=round((100-perc), 2),
                                             font=('calibre', 12), fg='black').grid(row=k+13, column=4)
                        ...
                        main_frame.pack(fill='both', expand=1)
                        nu = tk.IntVar()
                        tk.Label(main_frame, text='Nª iterations', font=(
                            'calibre', 10, 'bold'), padx=5, pady=5).grid(row=2, column=1)
                        tk.Entry(main_frame, textvariable=nu, font=(
                            'calibre', 10, 'normal')).grid(row=2, column=2)
                        tk.Button(main_frame, text='OK', command=nextt, font=(
                            'calibre', 10, 'bold'), padx=5, pady=5).grid(row=3, columnspan=2)
                    tk.Button(frame, text='Stochastic analysis', command=esto, font=(
                        'calibre', 10, 'bold'), padx=5, pady=5).grid(row=11, column=3)
                    ...
                    for k in range(0, len(classprediction)):
                        ...
                        if r == 'Risk':
                            tk.Label(frame, text=iD, font=('calibre', 12),
                                     fg='red').grid(row=k+13, column=1)
                            tk.Label(frame, text=r, font=('calibre', 12),
                                     fg='red').grid(row=k+13, column=2)
                            tk.Label(frame, text=tipp, font=('calibre', 12),
                                     fg='black').grid(row=k+13, column=3)
                        if r == 'No Risk':
                ...
                dis_btn = tk.Button(frame, text='Risk calculation',
                                    command=distancias, font=('calibre', 10, 'bold'))
            ...
            visor.btn = tk.Button(frame, text='See AoI', command=visor,
                                  font=('calibre', 10, 'bold'))
            visor.btn.grid(row=5, column=3, padx=5)

        affecciones_btn = tk.Button(second_frame, text='Upload AoI',
                                    command=locaff, font=('calibre', 10, 'bold'))
...
def cont():
    ...
    if not Bl.get() or not Wr.get() or not Lr.get() or not Slb.get() or not Slc.get() or not St.get() or not Lb.get() or not Manning.get() or not voll.get() or not x.get() or not y.get():
        ...
        title_label = tk.Label(second_frame, text='Drainage network',
                               font=('calibre', 12, 'bold'), padx=10, pady=10)
        ...
        tk.Button(second_frame, text='Open', command=dem, font=(
            'calibre', 10, 'bold')).grid(row=12, column=5, pady=8)
...
def geo():
    ...
    tk.Label(main_frame, text='Synthetic Geometry', font=(
        'calibre', 12, 'bold'), padx=5, pady=5).grid(row=0, columnspan=3)
    tk.Label(main_frame, text='Wr: Width reservoir (m)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=2, column=0)
    tk.Label(main_frame, text='Lr: Length reservoir (m)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=2, column=1)
    tk.Label(main_frame, text='Bl: Distance to the reservoir (m)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=3, column=0)
    tk.Label(main_frame, text='Slb: Longitudinal slope (B) (%)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=3, column=1)
    tk.Label(main_frame, text='Slc: Longitudinal slope (C)(%)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=4, column=0)
    tk.Label(main_frame, text='St: Transversal slope (B) C (%)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=4, column=1)
    tk.Label(main_frame, text='LB: Length Section B (m)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=5, column=0)
    tk.Label(main_frame, text='Wc: Channel width (m)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=5, column=1)
    tk.Label(main_frame, text='Hc: Depth channel (m)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=6, column=0)
    tk.Label(main_frame, text='Hr: Depth reservoir (m)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=6, column=1)
    tk.Label(main_frame, text='Volume:Volume reservoir (hm3)', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=7, column=0)
    tk.Label(main_frame, text='Manning: Roughness', font=(
        'calibre', 11, 'normal'), padx=5, pady=5).grid(row=7, column=1)
...
title_label = tk.Label(second_frame, text='Physical Parameters',
                       font=('calibre', 12, 'bold'), padx=10, pady=10)
