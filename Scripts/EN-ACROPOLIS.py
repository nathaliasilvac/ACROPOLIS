...
def dem():
    ...
    plot1.set_title('Elevation', fontsize=16, fontweight="bold")
    ...
    plot2.legend()
    plot2.set_title('Flow accumulation', fontsize=16, fontweight="bold")
    
    divider = make_axes_locatable(plot2)
    cax = divider.append_axes('right', size='5%', pad=0.08)
    fig.colorbar(im2, cax=cax, orientation='vertical', label='Nª Upstream cells')
    ...
    def locaff():
                ...
                class plot():
                    ...
                    ax1.scatter(main_path.path['x'], main_path.path['y'],s=5, label='Main drainage',color='brown')
                ...
                def ejes():
                    ...
                    ax1.set_title('Location AoI and axes', fontsize=16, fontweight="bold")
                    canvas = FigureCanvasTkAgg(fig1, master = frame)  
                    canvas.draw()
                    ...
                    #Al canal mainchannel
                        ...
                        grmat=material.loc[material['Riesgo']=='No Risk']
                        ...
                        cat_label=tk.Label(frame, text = 'Off-srtream reservoir category', font = ('calibre',10,'bold'))
                        ...
                        tk.Label(frame, text='Classification of risk', font = ('calibre',12,'bold')).grid(row=12,column=2)
                        tk.Label(frame, text='Type of AoI', font = ('calibre',12,'bold')).grid(row=12,column=3)
                ...
                eje_label=tk.Label(frame, text='Calculate axis',font = ('calibre',10,'bold'))
...
def cont():
    ...
    ttk.Label(second_frame, text = "Upload DEM", 
...
def geo():
    ...
    tk.Label( main_frame, text='Lr: Length reseervoir (m)',font = ('calibre',11,'normal'),padx=5, pady=5).grid(row=2, column=1)
    ...
    tk.Label( main_frame, text='St: Transversal slope (C) (%)',font = ('calibre',11,'normal'),padx=5, pady=5).grid(row=4, column=1)
    tk.Label( main_frame, text='LB: Length section B (m)',font = ('calibre',11,'normal'),padx=5, pady=5).grid(row=5, column=0)
    tk.Label( main_frame, text='Wc: Width channel (m)',font = ('calibre',11,'normal'),padx=5, pady=5).grid(row=5, column=1)
...
cont_bt=tk.Button(second_frame,text ='Ok', command=cont, font = ('calibre',10,'bold'))
